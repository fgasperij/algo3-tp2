...
Sea $ST_{i}$:Nat, el instante T de la recursion i.
...
Y para cada llamada recursiva a lo sumo puendo iterar por exactamente una vez por cada vuelo.
...
Luego la cantidad de iteraciones m\'axima que se realizan tiene una complejidad a lo usmo O($N^{2})$.
...
Todos los vuelos que salen de C y pertenecen a un itinerario valido, pueden ser tomados a partir de aterrizar en C con al menos un vuelo que aterrice en C y pertenezca a un itinerario valido.
...
Suponemos que no existe vuelos que ciclan a la misma ciudad, es decir que su destino y origen es el mismo.

Sea s un secuencia de vuelos, tal que s es un itinerario valido de vuelos minimos.

Para todo i:Nat, i<S.tamanio, $S_{i}$.aterrizaje < $S_{i+1}$.despegue o $S_{i}$.destino == final.

Luego si S contiene un ciclo, el vuelo que cierre ese ciclo, no va a cumplir este invariante, ya que el vuelo minimo es unico para cada ciudad.
