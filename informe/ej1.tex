...
Para construir el array de aeropuertos, lo que hacemos es contar la cantidad de ciudades para crear un array de ese tamaño.
Para ello, cargamos la ciudad en un TreeSet, como la cantidad de ciudades la podemos acotar por la cantidad de vuelos, ya que a lo sumo tenemos 2 ciudades por vuelo, sea N la cantidad de vuelos, la cantidad de ciudades maximas es de 2.N, por lo tanto el costo de crear este Set es a lo sumo O(n.Log(n))

Crear el array tiene una complejidad temporal de O(N) a lo sumo y espacial de 2.N.

Luego debemos asociarles un id a estas ciudades, que va a ser la posicion donde se van a encontrar en el array. Para poder asociarle este id construimos un mapa $\langle Ciudad,ID \rangle$.
Buscar en este mapa nos va a costar O(Log(N)).

Luego asociamos cada vuelo a la poscion donde se encuentran la ciudad destino e inicio de ese vuelo.
Para ello debemos buscar el id en nuestro mapa. Entonces para asociar todos los vuelos tenemos una complejidad de O(n.Log(n)).

Luego la complejidad final de construir nuestro array es O(n.Log(n)).

Para la funci\'on de recursi\'on vamos a evaluar cuantas iteraciones hay por cada recursi\'on que hacemos.
